...
import com.eproxim.multicloud.config.KafkaSseConsumer;
import com.eproxim.multicloud.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class MultiCloudManagerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/multi-cloud-manager-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/multi-cloud-manager-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/multi-cloud-manager-kafka/unregister"));
                return;
            }
        }
